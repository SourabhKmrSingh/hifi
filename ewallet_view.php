...
$closingDay = $configRow['closing_date'];
$closingDate = date("Y-m-{$closingDay}",strtotime($createdate));
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	if($data['num_rows'] >= 1)
	{
		foreach($data['result'] as $ewalletRow)
		{
			...
			$arrayClosingDate = explode("-", $closingDate);
			$arrayDataCreationDate = explode("-", $ewalletRow['createdate']);

			if($arrayClosingDate[0] == $arrayDataCreationDate[0] && $arrayClosingDate[1] == $arrayDataCreationDate[1] && $arrayClosingDate[2] > $arrayDataCreationDate[2]){
				continue;
			}
		?>
