...
$where_query .= " and type='credit' and regid='$regid'";
...
$url_parameters = "&userid=$userid&membership_id=$membership_id&reason=$reason&status=$status&month=$month&year=$year";
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	if($data['num_rows'] >= 1)
	{
		foreach($data['result'] as $ewalletRow)
		{
			...
			$creditResult = $db->view("SUM(amount) as total_credit_amount", "mlm_ewallet", "regid", "{$where_query}");
