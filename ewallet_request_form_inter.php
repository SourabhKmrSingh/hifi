...
$checkRequestDetialsQuery = $db->view('*','mlm_ewallet_requests','requestid'," and membership_id='$membership_id' and DATE_FORMAT(createdate, '%m-%Y') = DATE_FORMAT('{$createdate}', '%m-%Y')");


$dateCheck = date('d',strtotime($createdate));

if(!($dateCheck >= 6 && $dateCheck <= 12)){
	$_SESSION['error_msg'] = "You can only withdraw between 6 to 12 dates of every month.";
	header("Location: ewallet_request_form.php");
	exit();
}

if($checkRequestDetialsQuery['num_rows'] >= 1){
	$_SESSION['error_msg'] = "You can only withdraw once per month.";
	header("Location: ewallet_request_form.php");
	exit();
}
