...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">Plans <?php if($_SESSION['per_write'] == "1") { ?><a href="plan_form.php?mode=insert" class="btn mb_inline btn-sm btn_submit ml-3">Add New</a><?php } ?></h1>
	</div>
</div>

<form name="form_actions" method="POST" action="plan_actions.php" ENCTYPE="MULTIPART/FORM-DATA">
<div class="row">
	<div class="col-sm-12 mb-0">
		<div class="form-inline">
			...
			<a href="plan_view.php" class="btn  mb_inline btn-sm btn_delete ml-sm-2 ml-md-0 mb-2">Clear</a>
		</div>
	</div>
</div>
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	<tr>
		...
		<th class="<?php echo $th_sort1." ".$th_order_cls1; ?>"><a href="plan_view.php?orderby=title&order=<?php echo $th_order1; echo $url_parameters; ?>"><span>Title</span> <span class="sorting-indicator"></span></a></th>
		<th>Author</th>
		<th>Amount</th>
		<th>Status</th>
		<th class="<?php echo $th_sort3." ".$th_order_cls3; ?>"><a href="plan_view.php?orderby=createdate&order=<?php echo $th_order3.''.$url_parameters; ?>"><span>Date</span> <span class="sorting-indicator"></span></a></th>
	</tr>
	</thead>
	<tbody>
	<?php
	if($data['num_rows'] >= 1)
	{
		foreach($data['result'] as $planRow)
		{
		...
		<tr class="text-center has-row-actions">
			<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($planRow['planid']); ?>"/></td>
			<td data-label="Title - ">
				<a href="plan_form.php?mode=edit&planid=<?php echo $validation->db_field_validate($planRow['planid']); ?>" class="fw-500"><?php echo $validation->db_field_validate($planRow['title']); ?></a>
				
				<div class="row row-actions">
					<div class="col-sm-12">
						<?php if($_SESSION['per_update'] == "1") { ?>
							<a href="plan_form.php?mode=edit&planid=<?php echo $validation->db_field_validate($planRow['planid']); ?>">Edit</a>
							 | 
						<?php } ?>
						<?php if($_SESSION['per_delete'] == "1") { ?>
							<a href="plan_actions.php?q=del&planid=<?php echo $validation->db_field_validate($planRow['planid']); ?>" onClick="return del();" class="delete">Delete</a>
						<?php } ?>
					</div>
				</div>
			</td>
			<td data-label="Author - "><a href="plan_view.php?userid=<?php echo $userid; ?>"><?php echo $validation->db_field_validate($userRow['display_name']); ?></a></td>
			<td data-label="Amount - ">&#8377;<?php echo $validation->price_format($planRow['amount']); ?></td>
			<td data-label="Status - "><font color="<?php if($planRow['status'] == "active") { echo "green"; } else { echo "red"; } ?>"><?php echo $validation->db_field_validate(ucfirst($planRow['status'])); ?></font></td>
			<td class="date" data-label="Date - "><?php echo $validation->date_format_custom($planRow['createdate']); ?> <br class="mb-hidden" />(<?php echo $validation->timecount("{$planRow['createdate']} {$planRow['createtime']}"); ?>)</td>
		</tr>
		<?php
		}
	}
