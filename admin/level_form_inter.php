...
$intialPercentage = $validation->input_validate($_POST['intialPercentage']);
if($intialPercentage=='')
{
	$intialPercentage = 0;
}
$instParcentage = $validation->input_validate($_POST['instParcentage']);
if($instParcentage=='')
{
	$instParcentage = 0;
}
$numInst = $validation->input_validate($_POST['numInst']);
if($numInst=='')
{
	$numInst = 0;
}
...
$fields = array('planid'=>$planid, 'title'=>$title, 'title_id'=>$title_id, 'percentage'=>$percentage,
'intialPercentage'=> $intialPercentage, 'instParcentage'=> $instParcentage, 'numInst'=> $numInst, 'commission'=>$commission, 'order_custom'=>$order_custom, 'description'=>$description, 'imgName'=>$imgName, 'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
