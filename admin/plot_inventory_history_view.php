...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">Plots Inventory History <?php if($_SESSION['per_write'] == "1") { ?><a href="plot_inventory_history_form.php?mode=insert&inventoryid=<?php echo $inventoryid; ?>" class="btn mb_inline btn-sm btn_submit ml-3">Add New</a><?php } ?></h1>
	</div>
</div>
	...
	<div class="col-sm-12 mb-0">
		<div class="form-inline">
			...
			<select NAME="payment_type" CLASS="form-control mb_inline mb-2">
				...
				<option value="Closed" <?php if($payment_type == "Closed") echo "selected"; ?>>Closed</option>
			</select>
	...
	if($data['num_rows'] >= 1)
	{
		...
		?>
			<tr class="text-center has-row-actions">
				<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($inventoryhistoryRow['historyid']); ?>"/></td>
				<td data-label="Receipt No - ">
					<a href="plot_inventory_history_form.php?mode=edit&historyid=<?php echo $validation->db_field_validate($inventoryhistoryRow['historyid']); ?>&inventoryid=<?php echo $inventoryhistoryRow['inventoryid']; ?>" class="fw-500"><?php echo $validation->db_field_validate($inventoryhistoryRow['refno']); ?></a>
					
					<div class="row row-actions">
						<div class="col-sm-12">
							<?php if($_SESSION['per_update'] == "1") { ?>
								<a href="plot_inventory_history_form.php?mode=edit&historyid=<?php echo $validation->db_field_validate($inventoryhistoryRow['historyid']); ?>&inventoryid=<?php echo $validation->db_field_validate($inventoryhistoryRow['inventoryid']); ?>">View/Edit</a>
								| 
							<?php } ?>
							<?php if($_SESSION['per_delete'] == "1") { ?>
								<a href="plot_inventory_history_actions.php?q=del&historyid=<?php echo $validation->db_field_validate($inventoryhistoryRow['historyid']); ?>&inventoryid=<?php echo $validation->db_field_validate($inventoryhistoryRow['inventoryid']); ?>" onClick="return del();" class="delete">Delete</a>
							<?php } ?>
								|
								<a href="plot_inventory_history_invoice.php?historyid=<?php echo $validation->db_field_validate($inventoryhistoryRow['historyid']);?>">Download Receipt</a>
						</div>
					</div>
				</td>
				<td data-label="Author - "><a href="plot_inventory_history_view.php?userid=<?php echo $userid; ?>&inventoryid=<?php echo $inventoryhistoryRow['inventoryid']; ?>"><?php echo $validation->db_field_validate($userRow['display_name']); ?></a></td>
