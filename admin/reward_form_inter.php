...
$direct_sale = $validation->input_validate($_POST['direct_sale']);
if($direct_sale=='')
{
	$direct_sale = 0;
}
$group_sale = $validation->input_validate($_POST['group_sale']);
if($group_sale=='')
{
	$group_sale = 0;
}
$total_sale = $validation->input_validate($_POST['total_sale']);
if($total_sale=='')
{
	$total_sale = 0;
}
$incentive = $validation->input_validate($_POST['incentive']);
if($incentive=='')
{
	$incentive = 0;
}
$condition_rewardid = $validation->input_validate($_POST['condition_rewardid']);
if($condition_rewardid=='')
{
	$condition_rewardid = 0;
}
$condition_members = $validation->input_validate($_POST['condition_members']);
if($condition_members=='')
{
	$condition_members = 0;
}
$condition_legs = $validation->input_validate($_POST['condition_legs']);
if($condition_legs=='')
{
	$condition_legs = 0;
}
$salary = $validation->input_validate($_POST['salary']);
if($salary=='')
{
	$salary = 0;
}
$maintian = $validation->input_validate($_POST['maintian']);
if($maintian=='')
{
	$maintian = 0;
}
...
$fields = array('planid'=>$planid, 'title'=>$title, 'title_id'=>$title_id, 'direct_sale'=>$direct_sale, 'group_sale'=>$group_sale, 'total_sale'=>$total_sale, 'maintian'=> $maintian,'incentive'=>$incentive, 'condition_rewardid'=>$condition_rewardid, 'condition_members'=>$condition_members, 'condition_legs'=>$condition_legs, 'salary'=>$salary, 'order_custom'=>$order_custom, 'description'=>$description, 'imgName'=>$imgName, 'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
