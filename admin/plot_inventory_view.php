...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">Plots Inventories <?php if($_SESSION['per_write'] == "1") { ?><a href="plot_inventory_form.php?mode=insert" class="btn mb_inline btn-sm btn_submit ml-3">Add New</a><?php } ?></h1>
	</div>
</div>
...
<table class="table table-striped table-view plot-inventory-table">
	...
	<tr style='white-space: nowrap;'>
		...
		<th style='min-width: 8rem;' class="<?php echo $th_sort1." ".$th_order_cls1; ?>"><a href="plot_inventory_view.php?orderby=membership_id&order=<?php echo $th_order1; echo $url_parameters; ?>"><span>User</span> <span class="sorting-indicator"></span></a></th>
		...
		<th>Size</th>
		<th>Unit</th>
		<th  style='min-width: 9rem;' class="<?php echo $th_sort2." ".$th_order_cls2; ?>"><a href="plot_inventory_view.php?orderby=total_amount&order=<?php echo $th_order2; echo $url_parameters; ?>"><span>Total Amount</span> <span class="sorting-indicator"></span></a></th>
		<th>Paid</th>
		<th>Balance</th>
		<th>Percentage</th>
		<th>Payment Status</th>
		<th>Status</th>
		<th style='min-width: 8rem;' class="<?php echo $th_sort3." ".$th_order_cls3; ?>"><a href="plot_inventory_view.php?orderby=createdate&order=<?php echo $th_order3.''.$url_parameters; ?>"><span>Date</span> <span class="sorting-indicator"></span></a></th>
	</tr>
	</thead>
	<tbody>
	<?php
	if($data['num_rows'] >= 1)
	{
		foreach($data['result'] as $inventoryRow)
		{
			...
			$plotResult = $db->view("title,plotid,plot_size,units", "mlm_plots", "plotid", "and plotid='{$plotid}'");
			$plotRow = $plotResult['result'][0];
			
			$regid = $inventoryRow['regid'];
			$inventoryid = $inventoryRow['inventoryid'];
			
			$inventorytotalResult = $db->view("SUM(amount) as paid_amount", "mlm_plots_inventory_history", "historyid", "and inventoryid='{$inventoryid}' and regid='{$regid}'");
			$inventorytotalRow = $inventorytotalResult['result'][0];
			$paid_amount = $inventorytotalRow['paid_amount'];
			
			$inventorystatuslResult = $db->view("payment_type", "mlm_plots_inventory_history", "historyid", "and inventoryid='{$inventoryid}' and regid='{$regid}'", "historyid desc");
			$inventorystatusRow = $inventorystatuslResult['result'][0];
		?>
			<tr class="text-center has-row-actions">
				<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($inventoryRow['inventoryid']); ?>"/></td>
				<td data-label="User - ">
					...
					<div class="row row-actions">
						<div class="col-sm-12">
							...
							<br />
							<?php if($_SESSION['per_update'] == "1") { ?>
								<a href="plot_inventory_form.php?mode=edit&inventoryid=<?php echo $validation->db_field_validate($inventoryRow['inventoryid']); ?>">Edit</a>
								 | 
							<?php } ?>
							<?php if($_SESSION['per_delete'] == "1") { ?>
								<a href="plot_inventory_actions.php?q=del&inventoryid=<?php echo $validation->db_field_validate($inventoryRow['inventoryid']); ?>" onClick="return del();" class="delete">Delete</a>
							<?php } ?>
						</div>
					</div>
				</td>
				...
				<td data-label="Author - "><a href="plot_inventory_view.php?userid=<?php echo $userid; ?>"><?php echo $validation->db_field_validate($userRow['display_name']); ?></a></td>
				...
				<td data-label="Total Amount - "><?php echo $validation->db_field_validate($plotRow['plot_size']); ?></td>
				<td data-label="Total Amount - "><?php echo $validation->db_field_validate($plotRow['units']); ?></td>
				...
				<td data-label="Payment Status - "><?php echo $validation->db_field_validate($inventorystatusRow['payment_type']); ?></td>
				<td data-label="Status - "><font color="<?php if($inventoryRow['status'] == "active") { echo "green"; } else { echo "red"; } ?>"><?php echo $validation->db_field_validate(ucfirst($inventoryRow['status'])); ?></font></td>
	...
	?>
		<tr class="text-center">
			<td class="text-center" colspan="16">No Record is Available!</td>
		</tr>
	<?php
	}
	?>
