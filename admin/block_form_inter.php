...
$dupresult = $db->check_duplicates('mlm_blocks', 'blockid', $blockid, 'title_id', strtolower($title_id), $mode);
if($dupresult >= 1)
{
...
$fields = array('projectid'=>$projectid, 'title'=>$title, 'title_id'=>$title_id, 'amount'=>$amount, 'order_custom'=>$order_custom, 'description'=>$description, 'imgName'=>$imgName, 'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$blockQueryResult = $db->insert("mlm_blocks", $fields);
	if(!$blockQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$blockQueryResult = $db->update("mlm_blocks", $fields, array('blockid'=>$blockid));
	if(!$blockQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: block_view.php$search_filter");
	exit();
}
?>