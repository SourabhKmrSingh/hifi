...
<div class="form-rows-custom mt-3">
	<div class="row mb-3">
		...
		<div class="col-sm-9">
			<select NAME="planid" CLASS="form-control" ID="planid" required >
				...
				{
				?>
					<option VALUE="<?php echo $validation->db_field_validate($planRow['planid']); ?>" <?php if($mode == 'edit') { if($planRow['planid'] == $challengeRow['planid']) echo "selected"; } else { echo "selected"; } ?>><?php echo $validation->db_field_validate($planRow['title']); ?></option>
				<?php
				}
				?>
			</select>
	...
	<div class="row mb-3">
		<div class="col-sm-3">
			<label for="rewardids">Reward *</label>
		</div>
		<div class="col-sm-9">
			<select name="rewardids[]" id="rewardids" class="form-control multitextarea" multiple required >
				<?php
				$rewardids = explode(",", $challengeRow['rewardids']);
				...
				{
				?>
					<option VALUE="<?php echo $validation->db_field_validate($rewardRow['rewardid']); ?>" <?php if($mode == 'edit') { if(in_array($rewardRow['rewardid'], $rewardids)) echo "selected"; } ?>><?php echo $validation->db_field_validate($rewardRow['title']); ?></option>
				<?php
				}
				?>
			</select>
			<em class="d-block mt-1">Hold "Ctrl" key from your keyboard for Multi-selection</em>
		</div>
	</div>
	...
	<div class="row mb-3">
		...
		<div class="col-sm-9">
			<input type="number" name="direct_sale" id="direct_sale" class="form-control" value="<?php if($mode == 'edit') echo $validation->db_field_validate($challengeRow['direct_sale']); ?>" step="any" />
		</div>
	</div>
	...
	<div class="row mb-3">
		...
		<div class="col-sm-9">
			<input type="number" name="group_sale" id="group_sale" class="form-control" value="<?php if($mode == 'edit') echo $validation->db_field_validate($challengeRow['group_sale']); ?>" step="any" />
		</div>
	</div>
	...
	<div class="row mb-3">
		...
		<div class="col-sm-9">
			<input type="number" name="total_sale" id="total_sale" class="form-control" value="<?php if($mode == 'edit') echo $validation->db_field_validate($challengeRow['total_sale']); ?>" step="any" />
		</div>
	</div>
	...
	<div class="row mb-3">
		...
		<div class="col-sm-9">
			<input type="date" name="start_date" id="start_date" class="form-control" value="<?php if($mode == 'edit') echo $validation->db_field_validate($challengeRow['start_date']); ?>" />
		</div>
	</div>
	...
	<div class="row mb-3">
		...
		<div class="col-sm-9">
			<input type="date" name="end_date" id="end_date" class="form-control" value="<?php if($mode == 'edit') echo $validation->db_field_validate($challengeRow['end_date']); ?>" />
		</div>
	</div>
	...
	<div class="row mb-3">
		...
		<div class="col-sm-9">
			<textarea name="reward" id="reward" class="form-control"><?php if($mode == 'edit') echo $validation->db_field_validate($challengeRow['reward']); ?></textarea>
		</div>
	</div>
		...
		<div class="col-sm-12">
			...
			?>
				...
				<?php if($_SESSION['per_delete'] == "1") { ?>
					<a HREF="challenge_actions.php?q=del&challengeid=<?php echo $challengeRow['challengeid']; ?>" class="btn  btn-sm btn_delete" onClick="return del();"><i class="fas fa-trash"></i>&nbsp;&nbsp;Delete</a>
				<?php } ?>
			<?php
			}
			?>
