...
{
	$orderby_final = "transactionid desc";
}
...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">Transactions <!--<?php if($_SESSION['per_write'] == "1") { ?><a href="transaction_form.php?mode=insert" class="btn  btn-sm button">Add New</a><?php } ?>--></h1>
	</div>
</div>

<form name="form_actions" method="POST" action="transaction_actions.php" ENCTYPE="MULTIPART/FORM-DATA">
<div class="row">
	<div class="col-sm-12 mb-0">
		<div class="form-inline">
			...
			<a href="transaction_view.php" class="btn  mb_inline btn-sm btn_delete ml-sm-2 ml-md-0 mb-2">Clear</a>
		</div>
	</div>
</div>
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	<tr>
		...
		<th class="<?php echo $th_sort1." ".$th_order_cls1; ?>"><a href="transaction_view.php?orderby=refno&order=<?php echo $th_order1; echo $url_parameters; ?>"><span>Transaction ID</span> <span class="sorting-indicator"></span></a></th>
		<th>User</th>
		<th class="<?php echo $th_sort2." ".$th_order_cls2; ?>"><a href="transaction_view.php?orderby=amount&order=<?php echo $th_order2; echo $url_parameters; ?>"><span>Amount</span> <span class="sorting-indicator"></span></a></th>
		<th>Type</th>
		<th>Reason</th>
		<th>Description</th>
		<!--<th>Status</th>-->
		<th class="<?php echo $th_sort3." ".$th_order_cls3; ?>"><a href="transaction_view.php?orderby=createdate&order=<?php echo $th_order3.''.$url_parameters; ?>"><span>Date</span> <span class="sorting-indicator"></span></a></th>
	</tr>
	</thead>
	<tbody>
	<?php
	...
	if($data['num_rows'] >= 1)
	{
		foreach($data['result'] as $transactionRow)
		{
		?>
		<tr class="text-center has-row-actions">
			<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($transactionRow['transactionid']); ?>"/></td>
			<td data-label="Transaction ID - ">
				<?php echo $validation->db_field_validate($transactionRow['refno']); ?>
				
				<div class="row row-actions">
					<div class="col-sm-12">
						<?php if($_SESSION['per_delete'] == "1") { ?>
							<a href="transaction_actions.php?q=del&transactionid=<?php echo $validation->db_field_validate($transactionRow['transactionid']); ?>" onClick="return del();" class="delete">Delete</a>
						<?php } ?>
					</div>
				</div>
			</td>
			<td data-label="User - "><a href="transaction_view.php?regid=<?php echo $validation->db_field_validate($transactionRow['regid']); ?>"><?php echo $validation->db_field_validate($transactionRow['membership_id']); ?></a></td>
			<td data-label="Amount - ">&#8377;<?php echo $validation->price_format($transactionRow['amount']); ?></td>
			<td data-label="Type - "><?php echo $validation->db_field_validate(ucwords($transactionRow['type'])); ?></td>
			<td data-label="Reason - "><?php echo $validation->db_field_validate(ucwords($transactionRow['reason'])); ?></td>
			<td data-label="Description - "><?php echo $validation->db_field_validate($transactionRow['description']); ?></td>
			<!--<td data-label="Status - "><font color="<?php if($transactionRow['status'] == "approved" || $transactionRow['status'] == "fulfilled") { echo "green"; } else { echo "red"; } ?>"><?php echo $validation->db_field_validate(ucfirst($transactionRow['status'])); ?></font></td>-->
			<td class="date" data-label="Date - "><?php echo $validation->date_format_custom($transactionRow['createdate']); ?> <br class="mb-hidden" />(<?php echo $validation->timecount("{$transactionRow['createdate']} {$transactionRow['createtime']}"); ?>)</td>
		</tr>
		<?php
		}
	}
