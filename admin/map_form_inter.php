...
$dupresult = $db->check_duplicates('mlm_maps', 'mapid', $mapid, 'title_id', strtolower($title_id), $mode);
if($dupresult >= 1)
{
...
$fields = array('title'=>$title, 'title_id'=>$title_id, 'order_custom'=>$order_custom, 'description'=>$description, 'imgName'=>$imgName, 'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$mapQueryResult = $db->insert("mlm_maps", $fields);
	if(!$mapQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$mapQueryResult = $db->update("mlm_maps", $fields, array('mapid'=>$mapid));
	if(!$mapQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: map_view.php$search_filter");
	exit();
}
?>