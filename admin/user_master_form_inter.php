...
$dupresult = $db->check_duplicates('mlm_users', 'userid', $userid, 'username', strtolower($username), $mode);
if($dupresult >= 1)
{
...
if($imgTName != "")
{
	...
	if($mode == "edit")
	{
		$delresult = $media->filedeletion('mlm_users', 'userid', $userid, 'imgName', IMG_MAIN_LOC);
	}
}
...
if($mode == "insert")
{
	...
	$userQueryResult = $db->insert("mlm_users", $fields);
	if(!$userQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$userQueryResult = $db->update("mlm_users", $fields, array('userid'=>$userid));
	if(!$userQueryResult)
	{
		echo mysqli_error($connect);
		exit();
