...
if($title != "")
{
	...
	$where_query .= " and LOWER(title) LIKE '%$title1%'";
}
...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">News & Updates <?php if($_SESSION['per_write'] == "1") { ?><a href="news_updates_form.php?mode=insert" class="btn mb_inline btn-sm btn_submit ml-3">Add New</a><?php } ?></h1>
	</div>
</div>

<form name="form_actions" method="POST" action="news_updates_actions.php" ENCTYPE="MULTIPART/FORM-DATA">
<div class="row">
	<div class="col-sm-12 mb-0">
		<div class="form-inline">
			...
			<a href="news_updates_view.php" class="btn  mb_inline btn-sm btn_delete ml-sm-2 ml-md-0 mb-2">Clear</a>
		</div>
	</div>
</div>
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	<tr>
		...
		<th class="<?php echo $th_sort1." ".$th_order_cls1; ?>"><a href="news_updates_view.php?orderby=title&order=<?php echo $th_order1; echo $url_parameters; ?>"><span>Title</span> <span class="sorting-indicator"></span></a></th>
		<th>Message</th>
		<th>Status</th>
		<th class="<?php echo $th_sort3." ".$th_order_cls3; ?>"><a href="news_updates_view.php?orderby=createdate&order=<?php echo $th_order3.''.$url_parameters; ?>"><span>Date</span> <span class="sorting-indicator"></span></a></th>
	</tr>
	</thead>
	<tbody>
	<?php
	if($data['num_rows'] >= 1)
	{
		foreach($data['result'] as $news_updatesRow)
		{
			...
			$news_updatesUpdate = $db->update("mlm_news_updates", array('read_check'=> 1), array('newsid'=> $news_updatesRow['newsid']));
		?>
		<tr class="text-center has-row-actions ">
			<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($news_updatesRow['newsid']); ?>"/></td>
			<td data-label="Title - ">
				<a href="news_updates_form.php?mode=edit&newsid=<?php echo $validation->db_field_validate($news_updatesRow['newsid']); ?>" class="fw-500"><?php echo $validation->db_field_validate($news_updatesRow['title']); ?></a>
				
				<div class="row row-actions">
					<div class="col-sm-12">
						<?php if($_SESSION['per_update'] == "1") { ?>
							<a href="news_updates_form.php?mode=edit&newsid=<?php echo $validation->db_field_validate($news_updatesRow['newsid']); ?>">Edit</a>
							 | 
						<?php } ?>
						<?php if($_SESSION['per_delete'] == "1") { ?>
							<a href="news_updates_actions.php?q=del&newsid=<?php echo $validation->db_field_validate($news_updatesRow['newsid']); ?>" onClick="return del();" class="delete">Delete</a>
						<?php } ?>
						
					</div>
				</div>
			</td>
			<td data-label="Message - "><?php echo $validation->db_field_validate($news_updatesRow['message']); ?></td>
			<td data-label="Status - "><font class="<?php if($news_updatesRow['status'] == "inactive") { echo "status-red"; } else { echo "status-green"; } ?>"><?php echo $validation->db_field_validate(ucwords($news_updatesRow['status'])); ?></font></td>
			<td class="date" data-label="Date - "><?php echo $validation->date_format_custom($news_updatesRow['createdate']); ?> <br class="mb-hidden" />(<?php echo $validation->timecount("{$news_updatesRow['createdate']} {$news_updatesRow['createtime']}"); ?>)</td>
		</tr>
		<?php
		}
	}
