...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">Projects <?php if($_SESSION['per_write'] == "1") { ?><a href="project_form.php?mode=insert" class="btn mb_inline btn-sm btn_submit ml-3">Add New</a><?php } ?></h1>
	</div>
</div>

<form name="form_actions" method="POST" action="project_actions.php" ENCTYPE="MULTIPART/FORM-DATA">
<div class="row">
	<div class="col-sm-12 mb-0">
		<div class="form-inline">
			...
			<a href="project_view.php" class="btn  mb_inline btn-sm btn_delete ml-sm-2 ml-md-0 mb-2">Clear</a>
		</div>
	</div>
</div>
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	<tr>
		...
		<th class="<?php echo $th_sort1." ".$th_order_cls1; ?>"><a href="project_view.php?orderby=title&order=<?php echo $th_order1; echo $url_parameters; ?>"><span>Title</span> <span class="sorting-indicator"></span></a></th>
		<th>Author</th>
		<th>Amount</th>
		<th>Status</th>
		<th class="<?php echo $th_sort3." ".$th_order_cls3; ?>"><a href="project_view.php?orderby=createdate&order=<?php echo $th_order3.''.$url_parameters; ?>"><span>Date</span> <span class="sorting-indicator"></span></a></th>
	</tr>
	</thead>
	<tbody>
	<?php
	if($data['num_rows'] >= 1)
	{
		foreach($data['result'] as $projectRow)
		{
		...
		<tr class="text-center has-row-actions">
			<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($projectRow['projectid']); ?>"/></td>
			<td data-label="Title - ">
				<a href="project_form.php?mode=edit&projectid=<?php echo $validation->db_field_validate($projectRow['projectid']); ?>" class="fw-500"><?php echo $validation->db_field_validate($projectRow['title']); ?></a>
				
				<div class="row row-actions">
					<div class="col-sm-12">
						<?php if($_SESSION['per_update'] == "1") { ?>
							<a href="project_form.php?mode=edit&projectid=<?php echo $validation->db_field_validate($projectRow['projectid']); ?>">Edit</a>
							 | 
						<?php } ?>
						<?php if($_SESSION['per_delete'] == "1") { ?>
							<a href="project_actions.php?q=del&projectid=<?php echo $validation->db_field_validate($projectRow['projectid']); ?>" onClick="return del();" class="delete">Delete</a>
						<?php } ?>
					</div>
				</div>
			</td>
			<td data-label="Author - "><a href="project_view.php?userid=<?php echo $userid; ?>"><?php echo $validation->db_field_validate($userRow['display_name']); ?></a></td>
			<td data-label="Amount - ">&#8377;<?php echo $validation->price_format($projectRow['amount']); ?></td>
			<td data-label="Status - "><font color="<?php if($projectRow['status'] == "active") { echo "green"; } else { echo "red"; } ?>"><?php echo $validation->db_field_validate(ucfirst($projectRow['status'])); ?></font></td>
			<td class="date" data-label="Date - "><?php echo $validation->date_format_custom($projectRow['createdate']); ?> <br class="mb-hidden" />(<?php echo $validation->timecount("{$projectRow['createdate']} {$projectRow['createtime']}"); ?>)</td>
		</tr>
		<?php
		}
	}
