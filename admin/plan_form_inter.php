...
$tds = $validation->input_validate($_POST['tds']);
if($tds=='')
{
	$tds = 0;
}
$cash_incentive = $validation->input_validate($_POST['cash_incentive']);
if($cash_incentive=='')
{
	$cash_incentive = 0;
}
...
$dupresult = $db->check_duplicates('mlm_plans', 'planid', $planid, 'title_id', strtolower($title_id), $mode);
if($dupresult >= 1)
{
...
$fields = array('title'=>$title, 'title_id'=>$title_id, 'amount'=>$amount, 'order_custom'=>$order_custom, 'description'=>$description, 'imgName'=>$imgName, 'tds'=> $tds, 'cash_incentive'=> $cash_incentive,'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$planQueryResult = $db->insert("mlm_plans", $fields);
	if(!$planQueryResult)
	{
...
else if($mode == "edit")
{
	...
	$planQueryResult = $db->update("mlm_plans", $fields, array('planid'=>$planid));
	if(!$planQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: plan_view.php$search_filter");
	exit();
}
?>