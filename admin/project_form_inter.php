...
$dupresult = $db->check_duplicates('mlm_projects', 'projectid', $projectid, 'title_id', strtolower($title_id), $mode);
if($dupresult >= 1)
{
...
$fields = array('title'=>$title, 'title_id'=>$title_id, 'amount'=>$amount, 'order_custom'=>$order_custom, 'description'=>$description, 'imgName'=>$imgName, 'fileName'=>$fileName, 'priority'=>$priority, 'status'=>$status, 'user_ip'=>$user_ip);
...
if($mode == "insert")
{
	...
	$projectQueryResult = $db->insert("mlm_projects", $fields);
...
else if($mode == "edit")
{
	...
	$projectQueryResult = $db->update("mlm_projects", $fields, array('projectid'=>$projectid));
	if(!$projectQueryResult)
	{
		echo mysqli_error($connect);
		exit();
	...
	header("Location: project_view.php$search_filter");
	exit();
}
?>