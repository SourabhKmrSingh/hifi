...
<div CLASS="row">
	<div CLASS="col-lg-12">
		<h1 CLASS="page-header">Site Visit Requests</h1>
	</div>
</div>
...
<table class="table table-striped table-view" cellspacing="0" width="100%">
	...
	if($data['num_rows'] >= 1)
	{
		foreach($data['result'] as $sitevisitRow)
		{
			...
			$sitevisitUpdate = $db->update("mlm_site_visit", array('read_check'=> 1), array('sitevisitid'=> $sitevisitRow['sitevisitid']));
		?>
		<tr class="text-center has-row-actions <?php if($sitevisitRow['read_check'] == 0) echo "row-highlighted"; ?>">
			<td class="text-center" data-label=""><input type="checkbox" name="del_items[]" value="<?php echo $validation->db_field_validate($sitevisitRow['sitevisitid']); ?>"/></td>
			<td data-label="Name - ">
				<a href="site_visit_form.php?mode=edit&sitevisitid=<?php echo $validation->db_field_validate($sitevisitRow['sitevisitid']); ?>" class="fw-500"><?php echo $validation->db_field_validate($sitevisitRow['first_name'].' '.$sitevisitRow['last_name']); ?></a>
				
				<div class="row row-actions">
					<div class="col-sm-12">
						<?php if($_SESSION['per_update'] == "1") { ?>
							<a href="site_visit_form.php?mode=edit&sitevisitid=<?php echo $validation->db_field_validate($sitevisitRow['sitevisitid']); ?>">Edit</a>
							 | 
						<?php } ?>
						<?php if($_SESSION['per_delete'] == "1") { ?>
							<a href="site_visit_actions.php?q=del&sitevisitid=<?php echo $validation->db_field_validate($sitevisitRow['sitevisitid']); ?>" onClick="return del();" class="delete">Delete</a>
						<?php } ?>
